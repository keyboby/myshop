    ...
    <div class="col-md-12">
        <div ba-panel="" ba-panel-title="Editable Rows" ba-panel-class="with-scroll">
            <div class="panel panel-blur with-scroll animated zoomIn" zoom-in="" ba-panel-blur=""
                ...
                <div class="panel-heading clearfix"><h3 class="panel-title">车辆颜色列表</h3></div>
                <div class="panel-body" ng-transclude="">
                    <div include-with-scope="app/pages/tables/widgets/editableRowTable.html" class="ng-scope">
                                ...
                                <div class="panel panel-blur with-scroll animated zoomIn" zoom-in="" ba-panel-blur=""
                                    ...
                                    <div class="panel-body" ng-transclude="">
                                        <div ng-include="'app/pages/form/layouts/widgets/inlineForm.html'"
                                             class="ng-scope">
                                            <form class="row form-inline ng-pristine ng-valid ng-scope" action="{{ route('admin.car_color_save') }}" method="POST">
                                                @csrf
                                                <div class="form-group col-sm-3 col-xs-6">
                                                    <input type="text" class="form-control" name="car_color_name_add" placeholder="Name">
                                                </div>
                                                <div class="form-group col-sm-1 col-xs-6">
                                                    <select name="car_color_status_add" style="background-color:#FFFFFF; color:red;height: 35px;">
                        ...
                        <table class="table table-bordered table-hover table-condensed">
                            ...
                            @if (count($carColors))
                                @foreach ($carColors as $carColor)
                                    <tr ng-repeat="carColor in carColors" class="editable-row ng-scope">
                                        <td class="ng-binding">{{ $carColor->id }}</td>
                                        <td><span editable-text="user.name" e-name="name" e-form="rowform" e-required=""
                                                  class="ng-scope ng-binding editable">{{ $carColor->car_color_name }}</span>
                                        ...
                                        <td><span editable-text="user.created_at" e-name="created_at" e-form="rowform"
                                                  e-required=""
                                                  class="ng-scope ng-binding editable">{{ $carColor->created_at }}</span>
                                        </td>

                                        <td><span editable-text="user.status" e-name="status" e-form="rowform"
                                                  e-required=""
                                                  class="ng-scope ng-binding editable">{{ $carColor->status?'启用':'未启用' }}</span>
                                        ...
                                        <td>
                                            <form name="rowform" action="{{ route('admin.car_color_edit') }}"
                                                  method="get"
                                                  class="form-buttons form-inline ng-pristine ng-valid ng-hide"
                                                  style="display: none;">
                                                <input type="hidden" name="car_color_name" value="">
                                                <input type="hidden" name="status" value="">
                                                <input type="hidden" name="car_color_id" value="{{ $carColor->id }}">
                        ...
                        {{-- 分页 --}}
                        {!! $carColors->appends(Request::except('page'))->render() !!}
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('.btn_save').click(function () {
                ...
                _this.parent().find('input[name="car_color_name"]').val(car_brand_name);
